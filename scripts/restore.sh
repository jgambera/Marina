#!/bin/bash
# Artilect Database Restore Script
...
if [ $# -lt 1 ]; then
  ...
  ls -lh backups/artilect_*.db 2>/dev/null || echo "  (none found in backups/)"
  exit 1
fi

BACKUP_FILE="$1"
DB_PATH="${2:-${DB_PATH:-artilect.db}}"
...
echo ""
echo "Restart the Artilect server to use the restored database."
